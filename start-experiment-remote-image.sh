...
docker run -d -e HOST_USER_ID=$(id -u) -e HOST_GROUP_ID=$(id -g)  -v $EXECUTION_FOLDER/clonedRepositories:/usr/src/app/clonedRepositories -v $OUTPUT_FOLDER:/usr/src/app/output -v $PWD/input/projects.csv:/usr/src/app/projects.csv -v $PWD/input/commits.csv:/usr/src/app/commits.csv --name=$CONTAINER_NAME docker pull ghcr.io/jpedroh/experiment-last-merge:main ./tools/miningframework/install/miningframework/bin/miningframework --threads $THREADS --injector=injectors.GenericMergeModule --access-key=${GITHUB_ACCESS_KEY} --extension=".java" --log-level=TRACE --keep-projects --project-commit-hashes-file=commits.csv /usr/src/app/projects.csv
